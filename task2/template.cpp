...
template <int Number, int index = 100>
struct root
{
	static constexpr double X = root<Number, index - 1>::value;
	static constexpr double value = (X + Number / X) / 2;
	static const int value_int = static_cast<int>(value);
};

template <int Number>
struct root<Number, 0>
{
	static constexpr double value = Number;
};
...
// int euc = Print<euclid<30, 12>::value>::mew;
 int rt = Print<root<17>::value_int>::mew;
