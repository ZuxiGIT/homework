...
void Canvas::renderer()
{
    ...
    while(!SHOULD_STOP)
    {
        ...
        canvas_mutex.lock();
        for (unsigned line = 0; line < m_height; line+=2)
            for(unsigned column = 0; column < m_width; column+=2)
            {
        ...
        for (unsigned line = 0; line < m_height; line++)
            setPixel(m_width / 2, line, RGB(0, 0,0));
        
        for (unsigned column = 0; column < m_width; column++)
